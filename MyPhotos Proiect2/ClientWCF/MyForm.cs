    ...
    public partial class MyForm : Form
    {
        ...
        private void buttonAddTag_Click(object sender, EventArgs e)
        {
            ...
            if (tag.Type != "" && tag.Name != "")
            {
                if (listPhotos.SelectedIndex != -1)
                {
                    ...
                    controller.AddTagToPhoto(controller.GetPhotoByPath(listPhotos.SelectedItem.ToString()), controller.GetTag(tag));
                    listTags_Draw();
                }
            }
        }
        ...
        private void listTags_Draw()
        {
            if (listPhotos.SelectedIndex != -1)
            {
                ...
                foreach (Tag tag in controller.GetTagsOfPhoto(controller.GetPhotoByPath(listPhotos.Items[listPhotos.SelectedIndex].ToString())))
                {
        ...
        private void buttonAddFilter_Click(object sender, EventArgs e)
        {
            ...
            if (listPhotos.SelectedIndex != -1)
            {
                if (listTags.SelectedIndex != -1)
                {
                    ...
                    filters.Add(controller.GetTag(tag));
                    listFilters_Draw();
                    listPhotos_Draw();
                }
            }
        ...
        private void buttonRemoveFilter_Click(object sender, EventArgs e)
        {
            ...
            if (listPhotos.SelectedIndex != -1)
            {
                if (listFilters.SelectedIndex != -1)
                {
                    ...
                    filters.Remove(controller.GetTag(tag));
                    listFilters_Draw();
                    listTags_Draw();
                    listPhotos_Draw();
                }
